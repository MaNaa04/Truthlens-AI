"""
Gov Retriever - Layer 3B
Retrieves evidence exclusively from .gov domains.
"""
...
class GovRetriever:
    ...
    async def get_evidence(self, claim: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Gov retrieval failed for '{claim}': {e}")
            return None

    async def _fetch(self, client: httpx.AsyncClient, claim: str) -> Optional[str]:
        # Using site:.gov ensures we only get government sources
        params = {
            "q": f"{claim} site:.gov",
