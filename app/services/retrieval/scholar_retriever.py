"""
Scholar Retriever - Layer 3B
Retrieves evidence from Google Scholar (academic publications).
"""
...
class ScholarRetriever:
    ...
    async def get_evidence(self, claim: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Scholar retrieval failed for '{claim}': {e}")
            return None
    ...
    async def _fetch(self, client: httpx.AsyncClient, claim: str) -> Optional[str]:
        ...
        if "organic_results" in raw:
            for r in raw["organic_results"]:
                # Scholar results often have snippet inside publication_info or just snippet
                if "snippet" in r:
                    snippets.append(r["snippet"])
                elif "publication_info" in r and "summary" in r["publication_info"]:
                    snippets.append(r["publication_info"]["summary"])
